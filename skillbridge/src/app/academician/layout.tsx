...
const NAV_ITEMS = [
  { href: '/academician/dashboard', label: 'Faculty Hub', icon: LayoutDashboard },
  { href: '/academician/opportunities', label: 'FDP Programs', icon: BookOpen },
  { href: '/academician/research', label: 'Joint Research', icon: Microscope },
  { href: '/academician/mentorship', label: 'Mentorship', icon: Users },
];
  ...
  return (
    <div className="flex flex-col h-full bg-white border-r border-slate-200/80">
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {NAV_ITEMS.map((item) => {
          const active = pathname === item.href || (item.href !== '/academician/dashboard' && pathname.startsWith(item.href));
          return (
            <Link
              key={item.href}
              href={item.href}
